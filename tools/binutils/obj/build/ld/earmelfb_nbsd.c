...
static lang_output_section_statement_type *gldarmelfb_nbsd_place_orphan
  (asection *, const char *, int);

static void
gldarmelfb_nbsd_map_segments (bfd_boolean need_layout)
{
  int tries = 10;
...
gldarm_layout_sections_again (void)
{
  /* If we have changed sizes of the stub sections, then we need
     ...
     add even more stubs.  */
  gldarmelfb_nbsd_map_segments (TRUE);
  need_laying_out = -1;
}
...
gldarmelfb_nbsd_after_allocation (void)
{
  ...
  if (need_laying_out != -1)
    gldarmelfb_nbsd_map_segments (need_laying_out);
}
...
static bfd_boolean
gldarmelfb_nbsd_load_symbols (lang_input_statement_type *entry)
{
  int link_class = 0;
...
static void
gldarmelfb_nbsd_vercheck (lang_input_statement_type *s)
{
  const char *soname;
...
static void
gldarmelfb_nbsd_stat_needed (lang_input_statement_type *s)
{
  struct stat st;
  const char *suffix;
  const char *soname;
...
static bfd_boolean
gldarmelfb_nbsd_try_needed (struct dt_needed *needed,
				 int force)
{
  bfd *abfd;
  ...
  if (! force)
      ...
      if (needs != NULL)
	{
	  ...
	  lang_for_each_input_file (gldarmelfb_nbsd_vercheck);
	  if (global_vercheck_failed)
	    {
	      bfd_close (abfd);
  ...
  global_found = NULL;
  lang_for_each_input_file (gldarmelfb_nbsd_stat_needed);
  if (global_found != NULL)
    {
...
static bfd_boolean
gldarmelfb_nbsd_search_needed (const char *path,
				    struct dt_needed *n, int force, int prepend_sysroot)
{
  ...
  if (name[0] == '/') {
    if (prepend_sysroot && ld_sysroot) {
      ...
      rv = gldarmelfb_nbsd_try_needed (&needed, force);
      free(filename);
      return rv;
    }
    return gldarmelfb_nbsd_try_needed (n, force);
  }
  ...
  while (1)
      ...
      else
	{
      ...
      needed.name = filename;
      if (gldarmelfb_nbsd_try_needed (&needed, force))
	return TRUE;
...
static void
gldarmelfb_nbsd_check_needed (lang_input_statement_type *s)
{
  const char *soname;
...
gldarmelfb_nbsd_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldarmelfb_nbsd_write_build_id_section (bfd *abfd)
{
...
gldarmelfb_nbsd_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldarmelfb_nbsd_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldarmelfb_nbsd_write_build_id_section;
  ...
  for (l = needed; l != NULL; l = l->next)
	...
	continue;
      ...
      global_needed = l;
      global_found = NULL;
      lang_for_each_input_file (gldarmelfb_nbsd_check_needed);
      if (global_found != NULL
	  && (bfd_elf_get_dyn_lib_class (global_found->the_bfd)
	      & DYN_AS_NEEDED) == 0)
	continue;
      ...
      if (global_found != NULL)
	{
	  ...
	  if (gldarmelfb_nbsd_try_needed (&nn, TRUE))
	    continue;
	}
	 ...
	 their use.  See gldarmelfb_nbsd_vercheck comment.  */
      for (force = 0; force < 2; force++)
	{
	  size_t len;
	  search_dirs_type *search;

	  if (gldarmelfb_nbsd_search_needed (command_line.rpath_link,
						  &n, force, 0))
	    break;
	  len = strlen (l->name);
	  for (search = search_head; search != NULL; search = search->next)
		...
		continue;
	      ...
	      nn.name = filename;
	      if (gldarmelfb_nbsd_try_needed (&nn, force))
		break;
	      free (filename);
	    }
	  if (search != NULL)
...
static void
gldarmelfb_nbsd_find_exp_assignment (etree_type *exp)
{
  ...
  switch (exp->type.node_class)
      ...
      if (strcmp (exp->assign.dst, ".") != 0)
	{
      ...
      gldarmelfb_nbsd_find_exp_assignment (exp->assign.src);
      break;

    case etree_binary:
      gldarmelfb_nbsd_find_exp_assignment (exp->binary.lhs);
      gldarmelfb_nbsd_find_exp_assignment (exp->binary.rhs);
      break;

    case etree_trinary:
      gldarmelfb_nbsd_find_exp_assignment (exp->trinary.cond);
      gldarmelfb_nbsd_find_exp_assignment (exp->trinary.lhs);
      gldarmelfb_nbsd_find_exp_assignment (exp->trinary.rhs);
      break;

    case etree_unary:
      gldarmelfb_nbsd_find_exp_assignment (exp->unary.child);
      break;
...
static void
gldarmelfb_nbsd_find_statement_assignment (lang_statement_union_type *s)
{
  if (s->header.type == lang_assignment_statement_enum)
    gldarmelfb_nbsd_find_exp_assignment (s->assignment_statement.exp);
}


/* used by before_allocation and handle_option. */
static void 
gldarmelfb_nbsd_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
...
gldarmelfb_nbsd_before_allocation (void)
{
  ...
  lang_for_each_statement (gldarmelfb_nbsd_find_statement_assignment);
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldarmelfb_nbsd_append_to_separated_string (&depaudit, cp);
...
static bfd_boolean
gldarmelfb_nbsd_open_dynamic_archive
...
static lang_output_section_statement_type *
gldarmelfb_nbsd_place_orphan (asection *s,
				   const char *secname,
				   int constraint)
{
...
gldarmelfb_nbsd_handle_option (int optc)
{
  switch (optc)
	...
	link_info.emit_note_gnu_build_id = xstrdup (optarg);
      break;
    case OPTION_AUDIT:
  	gldarmelfb_nbsd_append_to_separated_string (&audit, optarg); 
	break;
    case 'P':
	gldarmelfb_nbsd_append_to_separated_string (&depaudit, optarg);
	break;
