				    ...
				    struct dt_needed *n, int force, int prepend_sysroot)
{
  ...
  if (name[0] == '/') {
    if (prepend_sysroot && ld_sysroot) {
      ...
      rv = gldarmelf_try_needed (&needed, force);
      free(filename);
      return rv;
    }
...
gldarmelf_id_note_section_size (bfd *abfd,
...
static bfd_boolean
gldarmelf_write_build_id_section (bfd *abfd)
{
...
gldarmelf_after_open (void)
{
  ...
  if (link_info.emit_note_gnu_build_id)
	...
	{
	  size = gldarmelf_id_note_section_size (abfd, &link_info);
	  if (size == 0)
	    {
	      ...
	      if (s != NULL && bfd_set_section_alignment (abfd, s, 2))
		{
		  ...
		  t->after_write_object_contents
		    = &gldarmelf_write_build_id_section;
